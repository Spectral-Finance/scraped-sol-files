...
/// @title ChainLinkTokenDualOracle
/// @author Drake Evans (Frax Finance) https://github.com/drakeevans
/// @notice  An oracle for ChainLinkToken
contract ChainLinkTokenDualOracle is
    DualOracleBase,
    Timelock2Step,
    UniswapV3SingleTwapOracle,
    ChainlinkOracleWithMaxDelay,
    EthUsdChainlinkOracleWithMaxDelay
{
    ...
    function name() external pure returns (string memory) {
        return "ChainLinkToken Dual Oracle Chainlink with Staleness Check and Uniswap V3 TWAP";
    }
    ...
    /// @notice The ```setTwapDuration``` function sets the twap duration for the ChainLinkToken V3 TWAP oracle
    /// @dev Requires msg.sender to be the timelock address
    ...
    /// @notice The ```getLinkPerUsdTwap``` function returns Link per USD using the ChainLinkToken V3 TWAP oracle & Chainlink
    ...
    function _getPrices() internal view returns (bool _isBadData, uint256 _priceLow, uint256 _priceHigh) {
        (bool _isBadDataChainlink, uint256 _linkPerUsdChainlink) = getLinkPerUsdChainlink();

        (bool _isBadDataTwap, uint256 _linkPerUsdTwap) = getLinkPerUsdTwap();
