...
import "src/AaveTokenDualOracle.sol";
...
import { deployAaveTokenDualOracle } from "script/deploy/DeployAaveTokenDualOracle.s.sol";
import { Mainnet as Constants_Mainnet, Arbitrum as Constants_Arbitrum } from "src/Constants.sol";

contract TestAaveTokenDualOracle is TestHelper {
    using OracleHelper for AggregatorV3Interface;

    AaveTokenDualOracle public dualOracle;
    uint256 ORACLE_PRECISION;

    function setUp() public {
        vm.createSelectFork(vm.envString("MAINNET_URL"), 16_867_280);

        (address _dualOracleAddress, , ) = deployAaveTokenDualOracle();

        dualOracle = AaveTokenDualOracle(_dualOracleAddress);
    ...
    function testGetPricesAaveToken() public {
