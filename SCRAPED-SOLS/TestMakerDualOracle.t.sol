...
import "src/MakerDualOracle.sol";
...
import { deployMakerDualOracle } from "script/deploy/DeployMakerDualOracle.s.sol";
import { Mainnet as Constants_Mainnet, Arbitrum as Constants_Arbitrum } from "src/Constants.sol";

contract TestMakerDualOracle is TestHelper {
    using OracleHelper for AggregatorV3Interface;

    MakerDualOracle public dualOracle;
    uint256 ORACLE_PRECISION;

    function setUp() public {
        vm.createSelectFork(vm.envString("MAINNET_URL"), 16_835_868);

        (address _dualOracleAddress, , ) = deployMakerDualOracle();

        dualOracle = MakerDualOracle(_dualOracleAddress);
    ...
    function testGetPricesMaker() public {
