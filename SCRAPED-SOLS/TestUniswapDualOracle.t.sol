...
import "src/UniswapDualOracle.sol";
...
import { deployUniswapDualOracle } from "script/deploy/DeployUniswapDualOracle.s.sol";
import { Mainnet as Constants_Mainnet, Arbitrum as Constants_Arbitrum } from "src/Constants.sol";

contract TestUniswapDualOracle is TestHelper {
    using OracleHelper for AggregatorV3Interface;

    UniswapDualOracle public dualOracle;
    uint256 ORACLE_PRECISION;

    function setUp() public {
        vm.createSelectFork(vm.envString("MAINNET_URL"), 16_835_868);

        (address _dualOracleAddress, , ) = deployUniswapDualOracle();

        dualOracle = UniswapDualOracle(_dualOracleAddress);
    ...
    function testStaleEthUsdData() public {
        ...
        _ethUsdOracle.setPrice(_price, uint256(block.timestamp - 4000), vm);
    ...
    function testGetPricesUniswap() public {
