...
import { iTokenCore, iTokenAdmin, iToken } from "../main.sol";

import { IWETH9 } from "../../interfaces/external/iWETH9.sol";
import { ILendingFactory } from "../../interfaces/iLendingFactory.sol";
import { IITokenAdmin, IITokenNativeUnderlying, IIToken } from "../../interfaces/iIToken.sol";
import { ILiquidity } from "../../../../liquidity/interfaces/iLiquidity.sol";

/// @dev overrides certain methods from the inherited iToken used as base contract to make them compatible with
/// the native token being used as underlying.
abstract contract iTokenNativeUnderlyingOverrides is iToken, IITokenNativeUnderlying {
    using FixedPointMathLib for uint256;

    /// @inheritdoc IITokenNativeUnderlying
    ...
    function rescueFunds(address token_) external virtual override(IITokenAdmin, iTokenAdmin) nonReentrant {
    ...
    /// @inheritdoc iTokenAdmin
    function rebalance()
        external
        payable
        virtual
        override(IITokenAdmin, iTokenAdmin)
        nonReentrant
...
abstract contract iTokenNativeUnderlyingActions is iTokenNativeUnderlyingOverrides {
    /*//////////////////////////////////////////////////////////////
                                DEPOSIT
    //////////////////////////////////////////////////////////////*/
    ...
    function mintNative(uint256 shares_, address receiver_) public payable nonReentrant returns (uint256 assets_) {
        ...
        if (msg.value < assets_) {
            ...
            revert FluidLendingError(ErrorTypes.iTokenNativeUnderlying__TransferInsufficient);
        }
...
/// @notice iTokens support EIP-2612 permit approvals via signature so withdrawals are possible with signature.
/// This contract implements those withdrawals for a native underlying asset.
abstract contract iTokenNativeUnderlyingEIP2612Withdrawals is iTokenNativeUnderlyingActions {
    /// @inheritdoc IITokenNativeUnderlying
    function withdrawWithSignatureNative(
        ...
        // @dev logic below is exactly the same as in {iTokenEIP2612Withdrawals-withdrawWithSignature}, just using
    ...
    function redeemWithSignatureNative(
        ...
        // @dev logic below is exactly the same as in {iTokenEIP2612Withdrawals-redeemWithSignature}, just using
        // _executeWithdrawNative instead of _executeWithdraw
...
/// @notice Same as the {iToken} contract but with support for native token as underlying asset.
/// Actual underlying asset is the wrapped native ERC20 version (e.g. WETH), which acts like any other iToken.
/// But in addition the iTokenNativeUnderlying also has methods for doing all the same actions via the native token.
contract iTokenNativeUnderlying is iTokenNativeUnderlyingEIP2612Withdrawals {
    ...
    constructor(
        ...
        IWETH9 weth_
    ) iToken(liquidity_, lendingFactory_, IERC20(address(weth_))) {}
    ...
    ) external virtual override(IIToken, iToken) {
        // not needed because msg.value is used directly
        revert FluidLendingError(ErrorTypes.iTokenNativeUnderlying__UnexpectedLiquidityCallback);
    }
