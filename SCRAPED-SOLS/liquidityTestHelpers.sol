...
abstract contract LiquiditySimulateStorageSlot {
    ...
    function _simulateUserSupplyDataFull(
        ...
        userSupplyData =
            ...
            (isUserPaused ? 1 : 0 << LiquiditySlotsLink.BITS_USER_SUPPLY_IS_PAUSED);
    ...
    function _simulateUserBorrowDataFull(
        ...
        /// Next  37 bits => 218-254 => empty for future use
        {
            ...
            baseLimit = BigMathMinified.toBigNumber(
                baseLimit,
                10,
                _DEFAULT_EXPONENT_SIZE,
                BigMathMinified.ROUND_DOWN
            );
            maxLimit = BigMathMinified.toBigNumber(
                maxLimit,
                10,
                _DEFAULT_EXPONENT_SIZE,
                BigMathMinified.ROUND_DOWN
        ...
        userBorrowData = userBorrowData | (isUserPaused ? 1 : 0 << LiquiditySlotsLink.BITS_USER_BORROW_IS_PAUSED);
...
abstract contract TestHelpers is LiquiditySimulateStorageSlot, Test {
    ...
    function _setDefaultTokenConfigs(address liquidityContract, address admin, address token) internal {
        ...
        vm.prank(admin);
        AdminModule(liquidityContract).updateTokenConfigs(tokenConfigs_);
    }
    ...
    function _setUserAllowancesDefaultWithMode(
        ...
        vm.prank(admin);
        AdminModule(liquidityContract).updateUserSupplyConfigs(userSupplyConfigs_);
        ...
        vm.prank(admin);
        AdminModule(liquidityContract).updateUserBorrowConfigs(userBorrowConfigs_);
    }
    ...
    function _setAsAuth(address liquidityContract, address admin, address user) internal {
        ...
        // execute
        vm.prank(admin);
        AdminModule(liquidityContract).updateAuths(updateAuthsParams);
    }
    ...
    function _pauseUser(
        ...
        // execute
        vm.prank(admin);
        AdminModule(liquidityContract).pauseUser(user, supplyTokens, borrowTokens);
    }
    ...
    function _unpauseUser(
        ...
        // execute
        vm.prank(admin);
        AdminModule(liquidityContract).unpauseUser(user, supplyTokens, borrowTokens);
    }
    ...
    function _warpWithExchangePriceUpdates(
        ...
        while (warpedSeconds < warpSeconds) {
            ...
            vm.prank(admin);
            AdminModule(liquidityContract).updateExchangePrices(tokens);
    ...
    function _assertSupplyRatio(LiquidityResolver resolver, address token, uint8 mode, uint256 ratio) internal {
    ...
    /// with `mode` being 0 if withInterest > interestFree, otherwise 1
    function _assertBorrowRatio(LiquidityResolver resolver, address token, uint8 mode, uint256 ratio) internal {
        uint256 exchangePricesAndConfig = resolver.getExchangePricesAndConfig(token);
    ...
    function _setUtilization(
        ...
        // vm.prank(user1);
        // // execute
        // UserModule(liquidityContract).operate(token, DEFAULT_SUPPLY_AMOUNT, 0, address(0), address(0), new bytes(0));
        // if (utilization > 0) {
        ...
        //     vm.prank(user2);
        //     // execute borrow
        //     UserModule(liquidityContract).operate(token, 0, borrowAmount_, address(0), user2, new bytes(0));
        // }
