...
contract VaultT1 is Helpers {
        ...
        )
    {
        ...
        if (o_.tick > type(int).min) {
            // fetch current debt
            o_.debtRaw = (TickMath.getRatioAtTick(int24(o_.tick)) * o_.colRaw) >> 96;
        ...
        if (o_.debtRaw > 0) {
            ...
            (o_.tick, o_.tickId, o_.debtRaw, o_.dustDebtRaw) = _addDebtToTickWrite(o_.colRaw, o_.debtRaw);
                ...
                if (o_.debtRaw > 0) {
                    ...
                    if (o_.tick > temp3_) {
                        unchecked {
                            ...
                            if (o_.oldTick <= o_.tick || (o_.debtRaw - o_.dustDebtRaw) > o_.oldNetDebtRaw) {
            ...
            if (newDebt_ < 0) {
                if (BORROW_TOKEN == NATIVE_TOKEN) {
                    unchecked {
                        temp_ = uint(-newDebt_);
                        if (msg.value > temp_) {
                            payable(msg.sender).transfer(msg.value - temp_);
    ...
    function liquidate(
        ...
        if (
            ...
            if (currentData_.debtRemaining > 0) {
                ...
                tickInfo_.ratio = TickMath.getRatioAtTick(int24(tickInfo_.tick));
                ...
                while (true) {
                    ...
                    debtLiquidated_ = uint(
                        // nominator
                        ((int((currentData_.refRatio * currentData_.debt) / currentData_.ratio) -
                            int(currentData_.debt)) * 1e27) /
                            // denominator
                            ((int(currentData_.colPerDebt * currentData_.refRatio) /
                                int(TickMath.ZERO_TICK_SCALED_RATIO)) - 1e27)
                    );
        ...
        if (BORROW_TOKEN == NATIVE_TOKEN) {
            ...
            if (actualDebtAmt_ < msg.value) {
                unchecked {
                    ...
                    payable(msg.sender).transfer(msg.value - actualDebtAmt_);
                }
            }
