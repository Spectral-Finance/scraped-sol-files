...
contract VaultT1Secondary is Variables, Error, Structs, Events {
    ...
    function absorb() public _verifyCaller {
...

        {
            ...
            (, , a_.supplyExPrice, a_.borrowExPrice) = IVaultT1(address(this)).updateExchangePrices(a_.vaultVariables2);
    ...
    function rebalance() external payable _verifyCaller returns (int supplyAmt_, int borrowAmt_) {
        ...
        IVaultT1.ConstantViews memory constants_ = IVaultT1(address(this)).constantsView();
        ILiquidity liquidity_ = ILiquidity(constants_.liquidity);
        RebalanceMemoryVariables memory r_;

        (r_.liqSupplyExPrice, r_.liqBorrowExPrice, r_.vaultSupplyExPrice, r_.vaultBorrowExPrice) = IVaultT1(
            address(this)
        ...
        if (totalSupplyVault_ > totalSupplyLiquidity_) {
            ...
            if (constants_.supplyToken == NATIVE_TOKEN) {
                if (msg.value > uint(supplyAmt_)) {
                    ...
                    payable(msg.sender).transfer(msg.value - value_); // sending back excess ETH
                } else {
