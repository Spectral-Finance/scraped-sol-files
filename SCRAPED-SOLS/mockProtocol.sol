...
import { UserModule } from "../liquidity/userModule/main.sol";
...
contract MockProtocol {
    ...
    function liquidityCallback(address token_, uint256 amount_, bytes memory data_) external {
        if (reentrancyFromCallback) {
            ...
            ILiquidity(liquidityContract).operate(token_, 10, 0, address(0), address(0), abi.encode(address(this)));
        }
    ...
    function operate(
        ...
        return
            UserModule(liquidityContract).operate{ value: valueAmount }(
                token_,
                supplyAmount_,
                borrowAmount_,
                withdrawTo_,
