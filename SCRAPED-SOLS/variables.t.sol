...
import { Variables, Constants } from "../../../contracts/protocols/lending/iToken/variables.sol";
...
contract VariablesTest is Test, RandomAddresses {
    ...
    function setUp() public {
        ...
        variables = new VariablesExposed(
            ILiquidity(liquidityAddress),
            ILendingFactory(lendingFactoryAddress),
            IERC20(asset)
        );
    }

    function test_initialState() public {
        assertEq(variables.name(), "Fluid Interest TestName", "Token name should be prefixed");
        assertEq(variables.symbol(), "fiTestSymbol", "Token symbol should be prefixed");
    }
...
contract VariablesExposed is Variables {
    ...
    function exposed_rewardsRateModel() external view returns (ILendingRewardsRateModel) {
    ...
    function updateRewards(ILendingRewardsRateModel) external pure override {
        revert("Not implemented");
    }
...
contract ConstantsExposed is Constants {
    ...
    function exposed_LIQUIDITY() external view returns (ILiquidity) {
        return LIQUIDITY;
    }

    function exposed_LENDING_FACTORY() external view returns (ILendingFactory) {
