...
// TODO: newer tests:
// 8. Initialize 2 users. 1 big user & another dust user. After 1st user withdraws his position, dust user won't be able to. Should throw this error: VaultT1__TickIsEmpty
// 9. Tests with multiple connection factors? We might already be doing it but can cross check on this
// 10. Payback or deposit when ratio is above CF. Should go through.
// 11. Try deposit & borrow above CF, final ratio should be less than before but still above CF (should fail)
// 12. Try payback & withdraw, final ratio should be equal to or less, should pass.
// 13. Try payback & withdraw, final ratio should be more, should fail.
...
import { VaultFactoryTest } from "../factory/vaultFactory.t.sol";
import { LiquidityResolver } from "../../../../contracts/periphery/resolvers/liquidity/main.sol";
import { VaultResolver } from "../../../../contracts/periphery/resolvers/vault/main.sol";
...
// TODO: newer tests:
// 1. Tests for vault with Native token support (2 types: native collateral token & native debt token)
// 2. Test Rebalance with native token as well.
// 3. Borrow above C.F (should fail)
// 4. Send msg.value > 0 for non native vault. Should throw this error: VaultT1__InvalidMsgValueOperate
// 5. Send msg.value different than collateral deposit or payback debt. Should throw
// 8. Initialize 2 users. 1 big user & another dust user. After 1st user withdraws his position, dust user won't be able to. Should throw this error: VaultT1__TickIsEmpty
// 9. Tests with multiple connection factors? We might already be doing it but can cross check on this
// 10. Payback or deposit when ratio is above CF. Should go through.
// 11. Try deposit & borrow above CF, final ratio should be less than before but still above CF (should fail)
...
// 16. [Admin] try absorbing dust debt of a liquidated position. Should fail.
// 17. [Admin] try absorbing dust debt of a liquidated position which is near 100% liquidated. Should fail.
// 18. Test rebalance with positive and negative ticks.
// 19. Test position with few wei borrowing such that it should shift to minimum tick.
// 20. Absorb such that new perfect topTick should be maxTick.
// 21. Absorb such that new liquidated topTick should be maxTick.
// 22. Verify positions when tick goes from +ve to -ve including 0.

contract VaultT1Test is VaultFactoryTest {
    VaultT1 vaultOne;
    VaultT1 vaultTwo;
    VaultT1 vaultThree; // Native supply vault
    VaultT1 vaultFour; // Native borrow vault
    address private constant nativeToken = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;
    ...
    LiquidityResolver liquidityResolver;
    ...
    function setUp() public virtual override {
        ...
        vaultOne = VaultT1(_deployVaultTokens(address(supplyToken), address(borrowToken)));
        vaultTwo = VaultT1(_deployVaultTokens(address(borrowToken), address(supplyToken)));
        vaultThree = VaultT1(_deployVaultTokens(address(nativeToken), address(borrowToken)));
        vaultFour = VaultT1(_deployVaultTokens(address(supplyToken), address(nativeToken)));
        ...
        liquidityResolver = new LiquidityResolver(ILiquidity(address(liquidity)));
        vaultResolver = new VaultResolver(address(vaultFactory), address(liquidity), address(liquidityResolver));
    ...
    function _deployVaultTokens(address supplyToken_, address borrowToken_) internal returns (address vault_) {
        ...
        vault_ = address(VaultT1(vaultFactory.deployVault(address(vaultT1Deployer), vaultT1CreationCode)));
    }

    function _setDefaultVaultSettings(address vault_) internal returns (address oracle_) {
        VaultT1Admin vaultAdmin_ = VaultT1Admin(vault_);
        vm.prank(alice);
    ...
    function _verifyLiquidation(
        ...
        VaultResolver.UserPosition memory userPosition_;
        VaultResolver.VaultEntireData memory vaultData_;
    ...
    function _verifyPosition() internal {
        (VaultResolver.UserPosition memory userPositionOne_, ) = vaultResolver.positionByNftId(1);
        (VaultResolver.UserPosition memory userPositionTwo_, ) = vaultResolver.positionByNftId(2);
    ...
    function _operateETHOnANonNativeVault(bool positiveTick_) public {
        ...
        VaultT1 vaultContract_ = VaultT1(vault_);
        VaultResolver.VaultEntireData memory vaultData_ = vaultResolver.getVaultEntireData(vault_);
    ...
    function testPaybackMoreThanBorrowed() public {
        ...
        // TODO: this should fail as debt should round up so paying back exactly should not throw above error
        vaultOne.operate(
            1, // new position
            0,
            -(7_990 * 1e18),
    ...
    function testDepositAndBorrowZero() public {
        VaultResolver.UserPosition memory userPosition_;
    ...
    function testChangeInExchangePrice() public {
        ...
        vm.prank(alice);
        VaultT1Admin(address(vaultOne)).updateSupplyRateMagnifier(vaultOneNewMagnifier_);
        vm.prank(alice);
        VaultT1Admin(address(vaultOne)).updateBorrowRateMagnifier(vaultOneNewMagnifier_);

        uint vaultTwoNewMagnifier_ = 9000;
        vm.prank(alice);
        VaultT1Admin(address(vaultTwo)).updateSupplyRateMagnifier(vaultTwoNewMagnifier_);
        vm.prank(alice);
        VaultT1Admin(address(vaultTwo)).updateBorrowRateMagnifier(vaultTwoNewMagnifier_);
    ...
    function testRebalanceNative() public {
        ...
        vm.prank(alice);
        VaultT1Admin(address(vaultThree)).updateSupplyRateMagnifier(m_.vaultThreeNewMagnifier);
        vm.prank(alice);
        VaultT1Admin(address(vaultThree)).updateBorrowRateMagnifier(m_.vaultThreeNewMagnifier);

        m_.vaultFourNewMagnifier = 9000;
        vm.prank(alice);
        VaultT1Admin(address(vaultFour)).updateSupplyRateMagnifier(m_.vaultFourNewMagnifier);
        vm.prank(alice);
        VaultT1Admin(address(vaultFour)).updateBorrowRateMagnifier(m_.vaultFourNewMagnifier);
    ...
    function _absorbAndUseAbsorbDustDebt(bool positiveTick_) public {
        ...
        nftIds_[2] = 3;
        vm.prank(alice);
        VaultT1Admin(vault_).absorbDustDebt(nftIds_);
